...
contract Base is Test, Fixtures {
    ...
    function setUp() public {
        STABLECOINS = [ // do not change the order here
            address(USDC), address(SUSDS), // < these two are deposited in Morpho
            address(GHO),  // < deposited in AAVE
            address(USDT), address(DAI),
            address(FRAX), address(USDE),
            address(USDS), address(CRVUSD),
        ...
        uint mainnetFork = vm.createFork("https://mainnet.base.org");
        vm.selectFork(mainnetFork);

        // Fund users with USDC via deal (no whale needed)
        ...
        AUX = new Aux(address(V4), address(CORE),
            address(gauntletWETHvault), address(AMP),
    ...
    function testDepositVaultShares() public {
        ...
        // Verify vault has the deposit (AuxBase: USDC at deposits[1])
        (uint[14] memory deposits, ) = AUX.get_deposits();
    ...
    function testVogueMultipleDeposits() public {
        ...
        assertApproxEqAbs(pooled3, 35 ether, 2, "Pooled should equal total deposited");
    ...
    function testFuzz_VogueDepositWithdraw(uint96 depositAmount, uint16 withdrawPct) public {
        ...
        if (toWithdraw > 0) {
            ...
            assertGt(received, toWithdraw * 97 / 100, "Received too little");
        }
        vm.stopPrank();
    }
